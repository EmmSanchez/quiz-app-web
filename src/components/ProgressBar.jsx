...
export function ProgressBar({ answeredQuestions, totalQuestions, isFinished }) {
    ...
    <div className="relative w-full px-6 mb-6 max-md:hidden">
      <div className="relative w-full h-2 rounded-full bg-zinc-900">
        <div
          ...
          className="h-2 bg-[#ffff3f] rounded-full transition-all ease-in-out duration-300"
        ></div>

        {/* Points Steps */}
        <div className="absolute -top-2 flex w-full justify-around">
          {questionsArray &&
            questionsArray.map((questionNumber, index) => {
              return (
                <div
                  key={index}
                  className="flex flex-1 justify-end transition-all duration-100"
                >
                  {isFinished ? (
                    <>
                    ...
                    <>
                      {index < answeredQuestions ? (
                        ...
                        <>
                          <p
                            className={`text-md px-2 rounded-full text-zinc-100 font-medium transition ease-in ${index <= answeredQuestions ? 'delay-150 bg-[#ffff3f] text-zinc-900' : 'bg-zinc-900'} ${index === answeredQuestions ? 'scale-110' : 'scale-100'}`}
                          >
                            {questionNumber}
                          </p>
                        </>
